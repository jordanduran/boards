...
const breadcrumbs = [
  { id: 1, name: 'Home', href: '/' },
  { id: 2, name: 'Complete Skateboards', href: '/complete-skateboards' },
];
...
const CompleteSkateboardProductPage = () => {
  const [completeSkateboard, setCompleteSkateboard] = useState(null);
  ...
  const handleSubmit = (e) => {
    ...
    completeSkateboard.id = completeSkateboardId;
    completeSkateboard.quantity = quantity;
    completeSkateboard.size = selectedSize;
    addToCart(completeSkateboard);
  ...
  useEffect(() => {
    if (completeSkateboard?.sizes?.length) {
      setSelectedSize(completeSkateboard?.sizes[0]);
    }
  }, [completeSkateboard]);
  ...
  useEffect(() => {
    const docRef = doc(db, 'completeSkateboards', completeSkateboardId);
    const getCompleteSkateboard = async () => {
      try {
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setCompleteSkateboard(docSnap.data());
        } else {
    ...
    getCompleteSkateboard();
  }, [completeSkateboardId]);

  if (completeSkateboard) {
    return (
        ...
        <div className='max-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8 lg:grid lg:grid-cols-2 lg:gap-x-8'>
            ...
            <section aria-labelledby='information-heading' className='mt-4'>
              ...
              <div className='flex items-center'>
                <p className='text-lg text-gray-900 sm:text-xl'>
                  ${completeSkateboard.displayPrice}
                </p>
              </div>

              <div className='mt-4 space-y-6'>
                <p className='text-base text-gray-500'>
                  {completeSkateboard.description}
                </p>
              </div>
          ...
          <div className='mt-10 lg:mt-0 lg:col-start-2 lg:row-span-2 lg:self-center'>
            <div className='aspect-w-1 aspect-h-1 rounded-lg overflow-hidden'>
              <img
                src={completeSkateboard.imageSrc}
                alt={completeSkateboard.name}
                className='w-9/12 h-full object-center object-cover m-auto'
              />
            </div>
          </div>
                ...
                <div className='sm:flex sm:justify-between'>
                  ...
                  <RadioGroup value={selectedSize} onChange={setSelectedSize}>
                    ...
                    <div className='mt-1 grid grid-cols-1 gap-4 sm:grid-cols-2 text-center'>
                      {completeSkateboard.sizes.map((size) => (
                        <RadioGroup.Option
...
export default CompleteSkateboardProductPage;
