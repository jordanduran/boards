...
const NewArrivalProductPage = () => {
  ...
  const { id: newArrivalId } = useParams();
  ...
  const handleSubmit = (e) => {
    ...
    newArrival.id = newArrivalId;
    newArrival.quantity = quantity;
    newArrival.size = selectedSize;
    addToCart(newArrival);
  ...
  useEffect(() => {
    if (newArrival?.sizes?.length) {
      setSelectedSize(newArrival?.sizes[0]);
    }
  }, [newArrival]);
  ...
  useEffect(() => {
    const docRef = doc(db, 'newArrivals', newArrivalId);
